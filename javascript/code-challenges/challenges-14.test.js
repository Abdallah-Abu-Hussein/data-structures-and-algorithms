...
const toTitleCase = (arr) => {
  ...
  let newArray=[];
  arr.map(value=>{
    newArray.push(value.charAt(0).toUpperCase()+ value.slice(1));
  });
  return newArray;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let newArray=[];
  let value=starWarsData[0].mass;
  arr.filter(element=>{
    if(parseInt(element.mass)>parseInt(value)){
...
const sortBy = (property, arr) => {
  ...
  arr.sort((val,inx)=>
  {
    if (val[property]>inx[property]) {
      return true;
    }
    else if(inx[property]>val[property])
    {
      return -1;
    }
    else
